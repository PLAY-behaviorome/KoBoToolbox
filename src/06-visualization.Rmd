...
```{r make-screening-site-func, message=FALSE}
...
```{r fig-screening-calls-map, fig.cap = "Families contacted by location"}
...
play_loco %>%
  dplyr::filter(., walk_mos_kea > 6, !is.na(walk_mos_kea)) %>%
  ggplot(.) +
...
play_loco %>%
  dplyr::filter(., walk_mos_who > 6, !is.na(walk_mos_who)) %>%
  ggplot(.) +
...
play_loco %>%
  dplyr::filter(., walk_mos_who > 6, !is.na(walk_mos_who), walk_mos_kea > 6, !is.na(walk_mos_kea)) %>%
  ggplot(.) +
