...
First we examine the variable names for each of the files.

```{r}
eng_18_1 <- readr::read_csv(eng_18_files$fn[1],
                    col_types = readr::cols(.default = 'c'),
                    show_col_types = FALSE)
...
```{r}
eng_18_2 <- readr::read_csv(eng_18_files$fn[2],
                    col_types = readr::cols(.default = 'c'),
                    show_col_types = FALSE)
...
```{r}
eng_18_3 <- readr::read_csv(eng_18_files$fn[3],
                    col_types = readr::cols(.default = 'c'),
                    show_col_types = FALSE)
...
Use the `basename()` function to trim the cruft.

```{r}
names(eng_18_1) |> basename()
```

```{r}
names(eng_18_2) |> basename()
```

```{r}
names(eng_18_3) |> basename()
```
```{r, eval=FALSE}
