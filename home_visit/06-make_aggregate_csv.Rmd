...
params:
  ...
  csv_save_dir: 'data/csv/release_1.0/aggregate'
...
if (length(fl) <= 0) warning("No files found in ", file.path(params$project_root_dir, params$csv_input_dir))
```
...
```{r}
PLAY_non_mbcdi_all <- PLAY_non_mbcdi_all %>%
  dplyr::distinct(.)
```
...
```{r export-aggregate}
aggregate_fn <- file.path(params$project_root_dir, params$csv_save_dir, params$no_db_fn)
readr::write_csv(PLAY_non_mbcdi_all, aggregate_fn)
```

## Remove redundant variable name text headers file-wide
