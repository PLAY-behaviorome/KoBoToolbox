...
tar_option_set(packages = c("readr", "dplyr", "ggplot2", "purrr", "tools", 
                            "httr", "stringr"))
list(
  tar_target(update_home, FALSE),
  tar_target(update_screening, FALSE),
  tar_target(clean_home_visit_data, clean_data_dir(update_home, home_visit_dir_xlsx)),
  tar_target(clean_demog_data, clean_data_dir(update_screening, screening_dir_xlsx)),
  ...
  tar_target(screening_downloads, retrieve_kobo_xlsx(kb_demog, screening_dir_xlsx)),
  tar_target(screening_xlsx_fns, list.files(screening_dir_xlsx, pattern = "[0-9]+.*\\.xlsx", full.names = TRUE)),
  ...
  #            format = "file"),
  tar_target(screening_xlsx_to_csv, load_xlsx_save_many_csvs(screening_dir_xlsx, screening_dir_csv, "Demographic")),
  ...
  tar_target(home_visit_xlsx_fns, list.files(home_visit_dir_xlsx, pattern = "[0-9]+.*\\.xlsx", full.names = TRUE)),
  tar_target(home_visit_csv_fns, list.files(home_visit_dir_csv, pattern = "[0-9]+.*\\.csv", full.names = TRUE)),
