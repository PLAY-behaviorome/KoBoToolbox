...
To download and clean *all* of the data, execute `rmarkdown::render("release_1.0_export_clean.Rmd", params = list(update_2020 = TRUE, update_2021 = TRUE))` from the console. Otherwise, run with the default parameter values: `rmarkdown::render("release_1.0_export_clean.Rmd")` which downloads the 2021+ Excel data files each time the document is run.
...
if ((dim(kb_home) < 1) || (!is.data.frame(kb_home))) {
...
bulk_download_kobo_forms(
  kb_home,
  '2020',
  update_form = params$update_2020,
...
As of 2022-03-09, the 2021 forms are the active ones. So, we have set `params$update_2021` == `r params$update_2021` by default.

```{r download-2021-plus}
bulk_download_kobo_forms(
  kb_home,
  '2021',
  update_form = params$update_2021,
  download_xlsx = params$download_xlsx,
  download_dir = params$xlsx_download_dir
)
```
