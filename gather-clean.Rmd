...
demog <- xlsx::read.xlsx(demog_fn_full, sheetIndex = 1)
demog <- as.tibble(demog)
```
...
```{r}
demog_clean %>%
  dplyr::arrange(., play_site_id, child_sub_id) %>%
...
1. Extract the site id and session or participant id
2. Save the row as a CSV with a name like `<site-id>-<session-id>.csv`.

```{r}
child_ids <- unique(demog_clean$child_play_id)

select_export_session <- function(this_child_id, df, 
  ...
  this_session <- dplyr::filter(df, child_play_id == this_child_id)
  
  if (export_file) {
    if (!is_empty(this_session)) {
      session_fn <- paste0(this_session$play_site_id, "-", 
                           this_session$child_sub_id, ".csv")
...
In trying to run this code across the set of unique `child_sub_id`s, the following code fails:

```{r, eval=FALSE}
purrr::map(child_ids, select_export_session, demog_clean)
```

I think we need to address some of the anomalies in the exported file to fix the error.

This shows some of what we need to address:

```{r}
demog_clean %>%
  dplyr::arrange(., play_site_id, child_play_id) %>%
  dplyr::select(., play_site_id, child_play_id, child_sub_id) %>%
  kableExtra::kable(., format = 'html')
```

</br>
</br>
Note that there is a duplicate record for Georgetown 006, Georgetown subs 005 and 007 seem to have the same identifier, and the identifier we generate sometimes has an initial slash '/' character.

We should also process the site ids so that the file names do not have spaces.
